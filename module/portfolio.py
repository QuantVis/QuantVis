...
def portfolio_result() :
    if session.get('email')==None:
        ...
        K_total_price_fi = format(round(K_total_price + round(F_total_price*exchng,2)),',')
        F_total_price_fi = format(round(F_total_price + round(K_total_price/exchng,2)),',')
...
def portfolio_list() :
    if session.get('email')==None:
        ...
        email = session.get('email')
        port = Portfolio()
        pfname_list = port.select_port_list(email)
        return render_template('portfolio_list.html',pfname_list=pfname_list)

@port.route('/delete',methods=['POST'])
def portfolio_list_del():
    if session.get('email')==None:
        flash('잘못된 접근입니다. 로그인 후 이용해주세요.')
        return redirect(url_for('user_login.login'))
    else:
        pfname_list = request.form.getlist('pfname')
        port = Portfolio()
        for pfname in pfname_list:
            port.delete_port(pfname)
        return redirect(url_for('portfolio.portfolio_list'))